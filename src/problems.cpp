...
std::vector<Conserved> updateSphericalLaxFriedrichs(const std::vector<Conserved>& grid, double dt) {
    ...
    for (int i = 1; i < N_ZONES - 1; ++i) {
        ...
        double r = X_MIN + (i + 0.5) * DX;   // Cell center
        double dt_half = 0.5 * dt;

        if (r > 1e-10) {
            // Only momentum has a non-zero source term
            double source_mom = 2.0 * w.p / r * dt_half;
            next_grid[i].mom += source_mom;
            // mass and energy unchanged (source terms are zero)
        }
    }
    ...
    // (At the inner boundary, v=0 will be enforced separately in Step 7.)
    ...
    // Indices: i=0  (left ghost - cell 0), ..., i=N_ZONES (cell N-1 - right ghost)
    ...
    for (int i = 0; i < N_ZONES; ++i) {
        next_grid[i].mass     = next_grid[i].mass     - dtdx * (F[i + 1].mass     - F[i].mass);
        next_grid[i].mom      = next_grid[i].mom      - dtdx * (F[i + 1].mom      - F[i].mom);
        next_grid[i].energy   = next_grid[i].energy   - dtdx * (F[i + 1].energy - F[i].energy);
    }
